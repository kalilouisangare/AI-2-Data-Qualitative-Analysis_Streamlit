...
async def upload_and_process_file(file: UploadFile = File(...), language: str = Form("auto")):
    try:
        ...
        elif filename.endswith('.docx'): doc = docx.Document(io.BytesIO(content_bytes)); current_context = "\n\n".join([p.text.strip() for p in doc.paragraphs if p.text.strip()])
        elif filename.endswith('.txt'): current_context = content_bytes.decode('utf-8', errors='ignore')
        elif filename.lower().endswith('.pdf'): doc = fitz.open(stream=content_bytes, filetype="pdf"); current_context = "".join([page.get_text() for page in doc])
        ...
        else: return JSONResponse(status_code=400, content={"message": "Format de fichier non supporté."})
        
        if not current_context.strip(): return JSONResponse(status_code=400, content={"message": "Fichier vide ou contenu non extrait."})
        
        return {"message": "Fichier traité.", "full_text": current_context, "is_audio": is_audio_context}
    except Exception as e: return JSONResponse(status_code=500, content={"message": f"Erreur: {e}"})
    finally:
        if tmp_path and os.path.exists(tmp_path): os.remove(tmp_path)

async def long_document_streamer(analysis_type, context, mode, model_choice):
    if not context: yield "ERREUR : Le contexte est vide."; return
    ...
    if mode == "api":
        ...
        async for token in qa_service_instance.ask(final_prompt, "", mode, model_choice):
            yield token
    else: # mode == "local"
        ...
        final_response_stream = qa_service_instance.ask(final_prompt, "", mode, model_choice)
        current_final_text = ""
        async for token in final_response_stream:
            current_final_text += token
            yield progress_display + current_final_text
...
async def ask_question(question: str = Form(...), context: str = Form(...), mode: str = Form("local"), model_choice: str = Form("auto")):
    if not context: return JSONResponse(status_code=400, content={"message": "Le contexte est vide."})
...
async def summarize_context(context: str = Form(...), is_audio: bool = Form(...), min_length: int = Form(30), max_length: int = Form(150)):
    if not is_audio: return JSONResponse(status_code=400, content={"message": "Le résumé automatique est uniquement disponible pour les fichiers audio transcrits."})
    if not context: return JSONResponse(status_code=400, content={"message": "Le contexte est vide."})
    try:
